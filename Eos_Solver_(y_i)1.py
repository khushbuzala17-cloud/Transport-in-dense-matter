...
def mu_hadronic(nb, yn, yp, kFn, kFp):
    ...
    common0 = (
        a0*(nb/nsat)
        + b0*(nb/nsat)**gamma * rho**(gamma-1)
    )

    common1 = (
        a1*(nb/nsat)
        + b1*(nb/nsat)**gamma1 * rho**(gamma1-1)
    )
...
def mu_quark(kFu, kFd, kFs):

    mu_u = np.sqrt(kFu**2 + mu**2)
    mu_d = np.sqrt(kFd**2 + md**2)
    mu_s = np.sqrt(kFs**2 + ms**2)
...
def calculate_state(nb, yn, yp, ys, ye, f):
    ...
    mu_u, mu_d, mu_s = mu_quark(kFu, kFd, kFs)
...
def residuals(x, nb, eta):
    ...
    # Penalty for unphysical guesses
    penalty = 0.0
    if (
        any(val < -0.001 for val in [yn, yp, ys, ye, f]) or
        any(val > 1.001 for val in [yn, yp, ys, ye]) or
        f > 1.001
    ):
        penalty = 1e6 * (
            sum(abs(min(v, 0)) for v in [yn, yp, ys, ye, f]) +
            abs(max(f - 1.0, 0))
        )
    ...
    try:
        ...
        if ys > 1.0:
            ...
            print(f"ye = {ye:.6f}")
            print(f"f  = {f:.6f}")
            print(f"yu = {state['yu']:.6f}")
            print(f"yd = {state['yd']:.6f}")
            print("======================================\n")
        # ---------------------------------------
        ...
        # R2: μp = 2μu + μd − η(μeN − μeQ)
        R2 = state["mu_p"] - (
            2.0 * state["mu_u"] + state["mu_d"]
            - eta * (state["mu_eN"] - state["mu_eQ"])
        )

        # R3: μd = μu + ημeQ + (1−η)μeG
        R3 = state["mu_d"] - (
            state["mu_u"] + eta * state["mu_eQ"]
            + (1.0 - eta) * state["mu_eG"]
        )
        ...
        # R5: PN + ηPeN = PQ + ηPeQ
        R5 = (
            state["PN"] + eta * state["PeN"]
            - state["PQ"] - eta * state["PeQ"]
        )
...
if results:
    ...
    output_df.to_excel("final_eos_solved_output3.xlsx", index=False)
